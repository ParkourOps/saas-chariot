...
<template>
    <div v-for="(tier, tierIdx) in catalogue" :key="`${catalogueId}:tier-${tierIdx}`" class="collapse collapse-arrow mt-4 px-2 text-neutral first:mt-0 sm:px-10">
        ...
        <div class="collapse-title flex rounded-box bg-primary shadow-xl" :class="[{ 'opacity-80': selectedIndex !== tierIdx }, { 'mb-8': selectedIndex !== tierIdx && catalogue.length - 1 === tierIdx }]">
